// Generated from base44/functions/receiveMetaWebhook/entry.ts. Do not edit directly.
...
async function readJsonWithRawBody(req) {
...
async function verifyMetaSignature(req, rawBody, secretName = "META_APP_SECRET") {
...
Deno.serve(async (req) => {
  if (req.method === "GET") {
    ...
    const mode = url2.searchParams.get("hub.mode");
    const token = url2.searchParams.get("hub.verify_token");
    const challenge = url2.searchParams.get("hub.challenge") || "";
  ...
  for (const entry of payload.entry || []) {
    for (const event of entry.messaging || []) {
      ...
      const attachments = (inboundMessage?.attachments || []).map((attachment) => {
        ...
        const url2 = attachment.payload?.url || attachment.payload?.sticker_id || "";
        return url2 ? `${type}:${url2}` : type;
      });
