...
Deno.serve(async (req) => {
  ...
  // Parse optional params: since_id, status, limit
  ...
  for (const so of shopifyOrders) {
    ...
    const shippingAddr = so.shipping_address
      ? [so.shipping_address.address1, so.shipping_address.city, so.shipping_address.province, so.shipping_address.country].filter(Boolean).join(', ')
      : '';
    ...
    for (const li of (so.line_items || [])) {
      ...
      // Compute fulfilled qty from Shopify fulfillment_line_items
      let fulfilledQty = 0;
      ...
      const lineStatus = fulfilledQty === 0 ? 'unfulfilled'
        : fulfilledQty >= (li.quantity || 0) ? 'fulfilled'
        : 'partial';

      // Get latest tracking from fulfillments for this line
      let trackingNumber = '';
      let carrier = '';
      for (const f of (so.fulfillments || [])) {
        for (const fli of (f.line_items || [])) {
